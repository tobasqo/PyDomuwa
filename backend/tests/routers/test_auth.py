...
class TestAuth:
    ...
    async def test_login_for_access_token(
        ...
        response = await api_client.post(f"{self.path}login", data=user_data)  # type: ignore
    ...
    async def test_login_for_access_token_non_existing_user(
        ...
        response = await api_client.post(f"{self.path}login", data=user_data)
        response_data = response.json()
    ...
    async def test_read_non_existing_user(self, api_client: AsyncClient):
        response = await api_client.get(f"{self.path}me")
        response_data = response.json()
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, response_data

    async def test_create_user(self, api_client: AsyncClient):
    ...
    async def test_get_user_by_invalid_id(
        ...
        response = await api_client.get(f"{self.path}-1", headers=authorization_headers)
        response_data = response.json()
